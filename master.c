...
int main() {
    ...
    int fd_r_input[2], fd_w_input[2];       //pipes for input
    ...
    if (BB == 0) {
        ...
        close(fd_w_input[1]);
        char fd_str[80];
        sprintf(fd_str, "%d %d %d %d", fd_r_drone[1], fd_w_drone[0], fd_r_input[1], fd_w_input[0]);
    ...
    if (input == 0) {
        ...
        close(fd_w_input[0]);
        char fd_str[80];
        sprintf(fd_str, "%d %d", fd_r_input[0], fd_w_input[1]);
